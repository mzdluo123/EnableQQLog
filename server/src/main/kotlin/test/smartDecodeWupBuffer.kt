...
fun smartDecodeWupBuffer(wupBuffer: ByteArray): String? = buildString {
...
private fun ByteArray.decodeUni(): String? {
    return toReadPacket().withUse {
        val requestPacket = readJceStruct(RequestPacket.serializer())
        requestPacket.smartToString()
    }
}

private fun RequestPacket.smartToString(): String? = buildString {
    ...
    appendLine(kotlin.runCatching { sBuffer.uniSmartPrint(version?.toInt() ?: 3) }.getOrElse { sBuffer.toUHexString() })
}